  ...
  return (
    <div className="container animate-fade-in" style={{ display: 'flex', gap: '40px', marginTop: '20px' }}>
      ...
      <div style={{ flex: '1' }}>
        ...
        <hr style={{ borderTop: '1px solid #D5D9D9', borderBottom: 'none', marginBottom: '15px' }} />
        <div style={{ fontSize: '28px', color: '#B12704', marginBottom: '15px' }}>
          ${parseFloat(product.price).toFixed(2)}
        </div>
        <div>
          <h4>About this item:</h4>
          <p style={{ marginTop: '10px', lineHeight: '1.5' }}>{product.description}</p>
        </div>
      </div>

      {/* Right - Action Panel (Buy Box) */}
      <div style={{ flex: '0.5', minWidth: '250px' }}>
        <div style={{ border: '1px solid #D5D9D9', borderRadius: '8px', padding: '20px', background: '#fff' }}>
          ...
          <div style={{ color: product.stock > 0 ? '#007600' : '#B12704', fontSize: '18px', fontWeight: 'bold', marginBottom: '15px' }}>
            {product.stock > 0 ? 'In Stock.' : 'Currently unavailable.'}
          </div>
          
          <div style={{ marginBottom: '15px' }}>
            <label>Qty: </label>
            <select value={quantity} onChange={e => setQuantity(Number(e.target.value))} style={{ padding: '4px', borderRadius: '4px' }}>
          ...
          <button onClick={handleAddToCart} className="btn btn-yellow" style={{ width: '100%', marginBottom: '10px' }}>
            Add to Cart
          </button>
          
          <button onClick={handleBuyNow} className="btn btn-orange" style={{ width: '100%' }}>
            Buy Now
          </button>
        </div>
      </div>
